...
public class TestService {
     ...
     * @return
     */
    @EzCache(key = "'getTestBos_0' + #args[0] + '-' + #args[1] + #args[2] + '-' + #args[3]", expireTimeMillis = 10 * 1000)
    public List<TestBo> getTestBos_0() {
        return Arrays.asList(new TestBo("test1", 1L));
    }

    @EzCache(key = "'getTestBos_1' + ':' + #args[0] + '-' + #args[1] + '-' + #args[2] + '-' + #args[3]", expireTimeMillis = 60 * 60 * 1000)
